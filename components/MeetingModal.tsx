...
import {
    Dialog,
    DialogContent,
} from "@/components/ui/dialog" // ← fixed: 'from' moved outside braces, removed unused imports
...
const MeetingModal = ({ isOpen, onClose, title, className, children, handleClick, buttonText, image, buttonIcon }: MeetingModalProps) => {
  return (
    <Dialog open={isOpen} onOpenChange={onClose}> {/* ← added missing space */}
        <DialogContent className='flex w-full max-w-[520px] flex-col gap-6 border-none bg-dark-1 px-6 py-9 text-white'>
            <div className='flex flex-col gap-6'>
                ...
                <h1 className={cn('text-3xl font-bold leading-[42px]', className)}>{title}</h1> {/* ← moved inside the div */}
                {children}
